...
async function runBroadcast(bot, ctx, text) {
    ...
    while (processed < totalUsers) {
        ...
        const users = await index_js_1.UserModel.find(query, { tgId: 1 })
            .sort({ _id: 1 })
            .limit(BATCH_SIZE)
            .lean();
        if (users.length === 0)
        ...
        if (processed % 200 === 0 || processed >= totalUsers) {
            await index_js_1.BroadcastModel.updateOne({ messageId: broadcastId }, { $set: { delivered: activeBroadcast.delivered, failed: activeBroadcast.failed + activeBroadcast.blocked } });
        }
    ...
    await index_js_1.BroadcastModel.updateOne({ messageId: broadcastId }, { $set: { delivered: d, failed: f + b, status: "completed" } });
...
function setupBroadcast(bot, adminSet) {
    ...
    bot.command("bcast", async (ctx) => {
        ...
        if (!id)
            return ctx.reply("Usage: `/bcast <id>`", { parse_mode: "Markdown" });
